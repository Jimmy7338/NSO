...
> **2026-09-11 修订完成：** V10.3 的 `gain.observe` 版本遗漏已留存旧源码并由 V10.3.1 修复。定向测试、小规模闭环和同一批 40 分支重跑均完成；40/40 独立重放通过。预测—观测闭环机制通过，但 S 与关闭反馈仍为 8/8 相同轨迹，故反馈性能增量仍未通过。新增可见三维表面积乘以全局 F1 不保证新增表面本身重建正确，也不等于二维覆盖乘以三维质量；28.27%/18.07% 不能写成精度提升。详见 [反馈修订与指标契约](V10_3_1_FEEDBACK_CORRECTION_AND_METRIC_CONTRACT.md)。
...
| V10.3.1 | 修复动作收益预测已登记但 `gain.observe` 未消费的问题；补齐完整与关闭反馈检查 | 40/40 闭环与重放通过；S 对关闭反馈仍 8/8 相同 | 实现闭环成立，标量反馈对 S 的决策增量仍为零 |

V10.2 的通道隔离显示，“只保留历史语义视角反馈”逐场景等于完整反馈，“只保留实现率后验”逐场景等于关闭反馈，因此负作用来自位姿推断的资产观察消费。历史 V10.3 实际遗漏实现率结算，不能用于判断该通道。V10.3.1 修复后，反馈开启的相机后验从 0.5 更新至约 0.20--0.31，关闭反馈保持 0.5，但 S 的完整/关闭反馈仍为 8/8 相同轨迹和指标。因此能确认的是：反馈执行路径已闭合，当前全局相机实现率标量对 S 候选排序没有决策级增量；H3 仍未成立。
...
该乘积概括新增观察广度与最终全局地图质量，不会因为类别本身得到额外权重；但全局 F1 不能保证新增表面自身正确表达。V10.3.1 因此另报新增可见参考点的局部 recall 和 completeness 距离。该乘积一般不满足次模性，也不保证每次 TSDF 融合后单调，因此只作为外部评价指标，不直接冒充在线信息增益。
...
- V10.3.1 修复后 40 分支：`eval_results/cpu_semantic_inspection_v10_3_1_feedback_fix_20260911/`
- V10.3.1 独立重放：`eval_results/cpu_semantic_inspection_v10_3_1_replay_20260911/`
- V10.3.1 修订与指标契约：`docs/research/V10_3_1_FEEDBACK_CORRECTION_AND_METRIC_CONTRACT.md`

V10.1 的独立程序重放了 24/24 分支，并核对冻结源码、每帧传感器、最终地图、网格、指标、返航及日志 argmax；S 相对 G、N 均在 7/8 个历史改变实际选择序列。V10.3.1 的事后独立程序没有调用规划器或运行适配器，按保存动作重新生成40/40分支的物理传感器哈希和 TSDF，并复算二维覆盖、全局 precision/recall/F1、表面误差、检查联合指标、新可见局部完整度和精确锚点返航，全部通过。25% 完成下限仍是单一开发值，没有完成跨值敏感性或独立场景确认。
