...
This r1 revision applies the single authorized connectivity repair to r0:
move the inner long panel outward 0.4 m and extend its front return equally.
The r0 source/evidence remain frozen. No observability or efficacy is assumed.
Construction checks physical spacing/safety, not paid coverage or Q.
"""
...
VERSION_V25 = 'two-facility-physical-partitions-v25-r1'
...
def partition_boxes_v25(parent='D25-P00'):
    ...
    for asset_id,(cx,front) in enumerate(LAYOUTS_V25[parent]['fronts']):
        ...
        panel('inner_long', 1.8,2.0,-.8,2.4)
        panel('inner_front_return', .8,2.0,-.4,-.2)
...
class StaticGeometryErrorV25(ValueError):
    def __init__(self, audit):
        ...
        super().__init__('V25 r1 physical safety/spacing failed: '+str(audit))
...
class FacilityChoiceWorldV25(FacilityChoiceWorldV24_1):
    ...
    def __init__(self, parent='D25-P00', assignment='A_complex_B_simple',
        ...
        self.static_geometry_audit = dict(world_version=VERSION_V25,repair_count=1,
            repair_scope='inner_long x[1.8,2.0]; inner_front_return x[.8,2.0]; all other geometry unchanged',
            partition_count=len(panels),
