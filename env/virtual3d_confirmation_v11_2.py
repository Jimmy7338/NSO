...
DEFAULT_MANIFEST = (Path(__file__).resolve().parents[1]
                    / "configs/virtual3d/semantic_gain_v11_1_confirmation2_contexts.json")


def sensor_seed(outer_seed, context_id):
    token = f"nso-semantic-gain-v11-confirmation2/{outer_seed}/{context_id}".encode()
...
class ConfirmationContextV11(CompetitionContextV9):
    ...
    def __post_init__(self):
        ...
        if not self.context_id.startswith("F") or self.outer_seed not in range(413, 425):
        ...
        if self.prefix_step not in (70, 150) or self.depth_sigma_m not in (.01, .02):
...
def load_confirmation_contexts(path=None):
    ...
    if (manifest["schema_version"] != "semantic_gain_v11_1_confirmation2_contexts/1"
    ...
    expected_ids = [f"F{i:02d}" for i in range(12)]
...
def get_confirmation_context(context_id, path=None):
    ...
    endpoint = {70: ((5.3, 3.7), 1), 150: ((4.1, 4.7), 0)}[step]
    if row["mirror_x"]:
