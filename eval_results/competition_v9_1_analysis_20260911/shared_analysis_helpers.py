...
def replay_status(inputs, run, metadata, expected):
    ...
    execution_seal = inputs.declared_read(run, 'pre_execution_seal.json')
    seal_bound = row.get('pre_execution_seal_sha256') == inputs.note(run / 'pre_execution_seal.json')
    archive_bound = row.get('source_archive_sha256') == inputs.note(run / 'sources.zip')
    manifest_bound = row.get('artifact_manifest_sha256') == inputs.note(run / 'artifact_hashes.json')
    if 'source_sha256' in row:
        inventory_bound = row['source_sha256'] == metadata.get('source_sha256')
        inventory_mode = 'explicit_verifier_source_inventory'
    else:
        # archived_contract already checked every declared source payload.
        # The frozen verifier binds the same archive, manifest and source-bearing
        # execution seal. Repeating that dictionary in verification is redundant.
        inventory_bound = (seal_bound and archive_bound and manifest_bound
                           and execution_seal.get('source_sha256') == metadata.get('source_sha256'))
        inventory_mode = 'equivalent_archive_manifest_and_preexecution_source_inventory_chain'
    checks = {
        ...
        'manifest_binding': manifest_bound,
        'source_archive_binding': archive_bound,
        'source_inventory_binding': inventory_bound,
        'preexecution_seal_binding': seal_bound,
        'prepared_manifest_binding': row.get('prepared_manifest_sha256') == execution_seal.get('prepared_manifest_sha256'),
    ...
    return gate(True if complete else False if row.get('status') == 'failed' else None,
                ...
                source_inventory_evidence_mode=inventory_mode,
