...
METHODS = ('G', 'O', 'S', 'N', 'X', 'M')
PROTOCOL = 'configs/virtual3d/competition_v9_1_validation_protocol.json'
...
def analyze(run, output, review):
    ...
    require(protocol['physical_branch_limit'] == seal['physical_branch_limit'] == 44, 'branch limit')
    ...
    verification = replay_status(inputs, run, metadata, 44)
    if (run / 'verification.json').exists():
        replay_receipt = inputs.read(run / 'verification.json')
        require(replay_receipt['verifier_sha256'] == metadata['source_sha256']['scripts/replay_competition_v9_1.py'], 'replay implementation not frozen')
        require(replay_receipt['structural_review_sha256'] == seal['structural_review_sha256'], 'replay structural receipt differs')
    parents, arrangements = [protocol[k] for k in ('parent_order', 'arrangement_order')]
    selected = {family: {} for family in ('total', 'rate')}
    histories, rows, tables, evidence = [], [], {}, []
    expected = {f'{p}/{a}/candidate_{i:03d}/outcome.json' for p in parents for a in arrangements for i in protocol['candidate_ids_by_parent'][p]}
    ...
    for parent in parents:
        ...
        for arrangement in arrangements:
            ...
            require([r['group'] for r in routes] == protocol['candidate_roles_by_parent'][parent], 'candidate roles')
            ...
            for family, sf, cf in (('total', 'scores', 'selected_candidate_ids'), ('rate', 'rate_scores', 'rate_selected_candidate_ids')):
                ...
                for mode in METHODS:
                    require(len(scores[mode]) == len(ids) and all(finite(v) for v in scores[mode]), 'invalid score vector')
    ...
    gates = {'all_44_complete': gate(len(rows) == metadata['new_physical_branches'] == 44),
    ...
    oracles = {p: {k: paired_oracle(tables[(p, arrangements[0])], tables[(p, arrangements[1])], protocol['candidate_ids_by_parent'][p], k,
    ...
    result = {'schema_version': 'competition_v9_1_analysis/1',
        ...
        'objective_frozen_before_first_world': True, 'variable_pool_rule_revised_after_prefix_before_future': True, 'original_V9_six_role_failure_preserved': True, 'minimum_one_sided_sign_p': 0.0625,
    ...
    lines = ['# V9.1 新几何场景可变候选集验证', '', f"状态：`{result['status']}`。44 条路线，{result['paid_branch_actions']} 个后续付费动作，另含 1200 个独立前置采集动作。", '',
