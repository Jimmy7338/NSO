...
def worker(run, output, frozen):
    ...
    actual_return = tuple((*world.position, world.heading)) == anchor and world.collisions == 0
    require(close["returned_to_anchor"] == actual_return and close["failed"] == (not actual_return), "return/failure status")
    require(actual_return and world.collisions == 0, "integration fixture did not safely return")
