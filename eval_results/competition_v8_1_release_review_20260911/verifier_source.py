#!/usr/bin/env python3
"""Independent protocol-selected V8 / V8.1 preparation release gates.
...
def worker(args):
    ...
    protocol_path = metadata.get('protocol_path', 'configs/virtual3d/competition_v8_protocol.json')
    protocol = read(args.snapshot / protocol_path)
    revision = protocol.get('revision_provenance')
    candidate_module = revision['candidate_module'] if revision else 'nso/competition_candidates_v8.py'
    assert candidate_module in metadata['source_sha256']
    assert protocol['parent_order'] == ['P0', 'P1'] and protocol['arrangement_order'] == ['shelf_west', 'shelf_east']
    ...
    assert sha(args.snapshot / protocol['context_manifest']) == protocol['context_manifest_sha256']
    if revision:
        assert protocol['schema_version'] == 'competition_v8_1_protocol/1'
        assert candidate_module == 'nso/competition_candidates_v8_1.py'
        base = Path(metadata['base_preparation'])
        assert sha(base / 'artifact_hashes.json') == metadata['base_manifest_sha256']
        for name, digest in revision['original_bindings'].items(): assert sha(base / name) == digest
        assert read(base / 'metadata.json')['status'] == 'halted_structural_failure'
        for name in ('immutable_scorer', 'immutable_scoring_geometry_module'):
            assert sha(args.snapshot / revision[name]) == revision[name + '_sha256']
        # Reused evidence must match its original bytes; new prediction files
        # are deliberately excluded because changing the pool changes scores.
        for context_id in protocol['parent_order']:
            for arrangement in protocol['arrangement_order']:
                ...
                original = base / context_id / arrangement
                for name in ('fixture.json', 'prefix_map.npz', 'reference.npz'):
                    assert sha(folder / name) == sha(original / name)
                for path in (folder / 'prefix').rglob('*'):
                    if path.is_file(): assert sha(path) == sha(original / path.relative_to(folder))
