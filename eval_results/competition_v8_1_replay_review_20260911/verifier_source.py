#!/usr/bin/env python3
"""Independently replay the complete sealed V8.1 24-branch development run.
...
def worker(args):
    ...
    require(protocol['parent_order'] == ['P0', 'P1'] and protocol['arrangement_order'] == ['shelf_west', 'shelf_east'], 'finite history inventory changed')
    ...
    require(protocol['reference']['samples_requested'] == 32000 and seal['physical_branch_limit'] == 24, 'reference/branch limits')
    ...
    for context_id in protocol['parent_order']:
        ...
        for arrangement in protocol['arrangement_order']:
            ...
            config = CompetitionConfigV8(**fixture['config'])
            ...
            require([r['candidate_id'] for r in routes] == list(range(6)), 'missing/extra shared candidate IDs')
            compare([r['group'] for r in routes], protocol['pre_outcome_structural_gates']['candidate_roles_in_order'], 'fixed roles')
            ...
            for method, scores in prediction['scores'].items():
                require(len(scores) == 6 and np.isfinite(scores).all(), 'invalid sealed score vector')
                selected = max(range(6), key=lambda i: (scores[i], -routes[i]['cost'], -i))
            ...
            truth = None
            for route in routes:
                world = CompetitionWorldV8(context, arrangement, config)
    ...
    compare(summary, {'outcomes': all_outcomes, 'physical_branches': 24,
                      'paid_branch_actions': sum(r['paid_actions'] for r in reports),
                      'shared_prefix_actions_per_history': 150, 'unique_prefix_histories': 4}, 'complete shared physical outcome set')
    meta = read(args.run / 'metadata.json')
    require(meta['new_physical_branches'] == 24 and meta['paid_actions'] == sum(r['paid_actions'] for r in reports)
    ...
    for module_name in ('env.virtual3d_competition_v8', 'nso.semantic_completion_v3', 'utils.rgbd_contract'):
    ...
    write(args.result, {'branches': reports, 'physical_branches': 24, 'branches_checked': 24, 'branches_total': 24,
                        'paid_actions': sum(r['paid_actions'] for r in reports),
                        'unique_prefix_histories': 4, 'unique_prefix_paid_actions': 600,
                        'unique_prefix_rgbd_scan_pairs': 604, 'prefix_rgbd_scan_pair_comparisons': prefix_sensor_comparisons,
...
def main(args):
    ...
    report = {'status': 'failed', 'passed_full': False, 'partial': False, 'max_branches': None,
              'run': str(args.run), 'scope': 'all 24 shared V8.1 development branches; independent raw replay and accounting only'}
    try:
    ...
    if target.exists():
        ...
        retained = root / 'audit_results' / 'competition_v8_1_replay_attempts' / sha(target)
