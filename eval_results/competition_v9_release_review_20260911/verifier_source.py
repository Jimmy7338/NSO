#!/usr/bin/env python3
"""Independent prospective V9 eight-history preparation release gates.

No future branch is generated. The frozen mapper and sensor simulator are
reused; grouping, slot assignment, footprint/path checks, and total/rate selection arithmetic
are computed here. Actual X/M semantics are remapped from all saved frames. Truth is consulted only after the choice seal is verified,
...
def verify_rule_freeze(directory, snapshot, protocol_path, context_path):
    """Bind the reviewed immutable inputs to the pre-world rule receipt."""
    manifest = read(directory / 'artifact_hashes.json')
    for name, digest in manifest.items():
        target = (directory / name).resolve()
        assert target.is_relative_to(directory.resolve())
        assert sha(target) == digest, name
    receipt = read(directory / 'receipt.json')
    assert receipt['schema_version'] == 'competition_v9_preconstruction_rule_seal/1'
    expected = {name: sha(snapshot / name) for name in receipt['source_sha256']}
    assert expected == receipt['source_sha256']
    assert receipt['protocol_sha256'] == expected[protocol_path]
    assert receipt['context_manifest_sha256'] == expected[context_path]
    assert receipt['first_v9_world_constructed'] is False
    assert receipt['new_v9_future_branches'] == 0
    return {'directory': str(directory), 'manifest_sha256': sha(directory / 'artifact_hashes.json'),
            'receipt_sha256': sha(directory / 'receipt.json'), 'input_bindings': expected,
            'pre_world_receipt_zero_future_actions': True}


def verify_scores_and_actual_remaps(mapper, frames, scans, routes, predictions):
    """Rebuild X/M sensors; reuse frozen prior, independently check arithmetic."""
    import numpy as np
    from nso.competition_prior_v8 import score_routes
    from nso.semantic_completion_v3 import SemanticHistoryMapperV3
    methods = ('G', 'O', 'S', 'N', 'X', 'M')
    poses = [frame.world_from_camera for frame in frames]
    original = score_routes(mapper, routes, poses)
    assert not original['trained'] and not original['calibrated']
    assert not predictions['trained'] and not predictions['calibrated']
    assert set(predictions['scores']) == set(predictions['rate_scores']) == set(methods)
    assert predictions['audit'] == original['audit']
    costs = np.asarray([route['cost'] for route in routes], dtype=float)
    for mode in methods:
        rate = np.asarray(original['scores'][mode], dtype=float)
        assert np.isfinite(rate).all()
        np.testing.assert_array_equal(predictions['rate_scores'][mode], rate)
        np.testing.assert_array_equal(predictions['scores'][mode], rate * costs)
        for score_key, choice_key in (('scores', 'selected_candidate_ids'),
                                      ('rate_scores', 'rate_selected_candidate_ids')):
            score = predictions[score_key][mode]
            index = min(range(len(routes)), key=lambda i: (-score[i], routes[i]['cost'], routes[i]['candidate_id']))
            assert predictions[choice_key][mode] == routes[index]['candidate_id']
    for scores, choices in (('scores', 'selected_candidate_ids'),
                            ('rate_scores', 'rate_selected_candidate_ids')):
        np.testing.assert_array_equal(predictions[scores]['M'], predictions[scores]['G'])
        assert predictions[choices]['M'] == predictions[choices]['G']
    original_q = mapper.quality_evidence(max_points=10**9)
    assert original_q is not None
    original_groups = groups_from_raw_mapper(mapper, poses[0])
    remap_rows = []
    for condition, expected in (('shuffled', 'X'), ('absent', 'G')):
        changed = SemanticHistoryMapperV3(mapper.shape, mapper.config, mapper.config.truncation_m)
        for frame, scan in zip(frames, scans):
            labels = np.zeros_like(frame.semantic)
            if condition == 'shuffled':
                labels[frame.semantic == 2] = 3
                labels[frame.semantic == 3] = 2
            changed.update(replace(frame, semantic=labels), scan)
        np.testing.assert_array_equal(changed.belief, mapper.belief)
        np.testing.assert_array_equal(changed.camera_seen, mapper.camera_seen)
        changed_q = changed.quality_evidence(max_points=10**9)
        assert changed_q is not None and changed_q.keys() == original_q.keys()
        for key in original_q:
            if key != 'label':
                np.testing.assert_array_equal(changed_q[key], original_q[key], err_msg=key)
        changed_groups = groups_from_raw_mapper(changed, poses[0])
        assert len(changed_groups) == len(original_groups)
        for first, second in zip(original_groups, changed_groups):
            for key in first:
                if key not in ('marked_points', 'class_vote', 'marked_cloud'):
                    np.testing.assert_array_equal(first[key], second[key], err_msg=key)
        actual = score_routes(changed, routes, poses)
        np.testing.assert_array_equal(actual['scores']['S'], predictions['rate_scores'][expected])
        np.testing.assert_array_equal(np.asarray(actual['scores']['S']) * costs,
                                      predictions['scores'][expected])
        remap_rows.append({'condition': condition, 'actual_raw_frames_remapped': len(frames),
                           'nonsemantic_map_and_quality_support_exact': True,
                           'independently_rebuilt_candidate_geometry_exact': True,
                           'actual_S_matches_recorded': expected,
                           'rate_and_total_scores_exact': True})
        del changed
    return {'all_twelve_score_arrays_recomputed': True,
            'all_twelve_tie_broken_choices_independently_checked': True,
            'total_equals_rate_times_same_planned_cost_exact': True,
            'M_equals_G_scores_and_choices_exact': True,
            'frozen_prior_reused_not_independently_implemented': True,
            'new_scores_or_routes_selected_using_future_outcomes': False,
            'actual_remaps': remap_rows}
...
def worker(args):
    ...
    protocol_path = metadata.get('protocol_path', 'configs/virtual3d/competition_v9_validation_protocol.json')
    protocol = read(args.snapshot / protocol_path)
    assert protocol['schema_version'] == 'competition_v9_validation_protocol/1'
    candidate_module = 'nso/competition_candidates_v8_1.py'
    assert candidate_module in metadata['source_sha256']
    assert protocol['parent_order'] == ['Q0', 'Q1', 'Q2', 'Q3']
    assert protocol['arrangement_order'] == ['shelf_west', 'shelf_east']
    assert protocol['branch_actions'] == 48 and protocol['prefix_paid_actions'] == 150
    assert protocol['physical_branch_limit'] == 48 and protocol['physical_history_count'] == 8
    assert protocol['pre_outcome_structural_gates']['candidate_target_max_distance_error_m'] == .35
    assert not protocol['pre_outcome_structural_gates']['deep_region_visit_and_optical_exclusivity_required']
    context_manifest = read(args.snapshot / protocol['context_manifest'])
    assert context_manifest['schema_version'] == 'competition_v9_contexts/1'
    assert [row['context_id'] for row in context_manifest['contexts']] == protocol['parent_order']
    expected_parents = {row['context_id']: row for row in protocol['prospective_world_contract']['parents']}
    common = protocol['prospective_world_contract']['common_geometry']
    for key in ('room_width_m', 'room_height_m', 'object_height_m', 'plinth_height_m',
                'panel_thickness_m', 'storage_board_z_m', 'storage_board_thickness_m',
                'marker_width_m', 'marker_z_bounds_m'):
        assert context_manifest['geometry'][key] == common[key], key
    for row in context_manifest['contexts']:
        for key, value in expected_parents[row['context_id']].items():
            assert row[key] == value, (row['context_id'], key)
        assert row['outer_seed'] not in protocol['excluded_outer_seeds']
        for key in ('object_front_centers_xy_m', 'initial_xy_m', 'initial_heading',
                    'final_prefix_xy_m', 'final_prefix_heading'):
            assert row[key] == common[key], key
        token = f"nso-competition-v9/{row['outer_seed']}/{row['context_id']}/common-sensor-stream".encode()
        assert row['sensor_seed'] == int.from_bytes(hashlib.sha256(token).digest()[:4], 'little')
    for name, digest in protocol['provenance']['unchanged_dependencies'].items():
        assert sha(args.snapshot / name) == digest, name
    freeze_review = verify_rule_freeze(args.freeze, args.snapshot, protocol_path, protocol['context_manifest'])
    ...
    for context_id in protocol['parent_order']:
        for arrangement in protocol['arrangement_order']:
            base = f'{context_id}/{arrangement}/'
            required = [base + name for name in ('fixture.json', 'candidates.json', 'candidate_audit.json',
                                                  'predictions.json', 'prefix_map.npz', 'prefix/records.json')]
            required += [f'{base}prefix/{kind}/{index:04d}.npz'
                         for kind in ('frames', 'scans') for index in range(151)]
            for name in required:
                assert name in choice_seal and choice_seal[name] == manifest[name], name
    reports, pairs, raw_count = [], {}, 0
    for context_id in protocol['parent_order']:
        pair = []
        for arrangement in protocol['arrangement_order']:
            folder = args.run / context_id / arrangement
            fixture = read(folder / 'fixture.json'); c = CompetitionConfigV9(**fixture['config'])
            context = CompetitionContextV9(**fixture['context'])
            world = CompetitionWorldV9(context, arrangement, c)
            ...
            # Missing roles are a scientific structural failure, not a
            # reviewer crash: verify every retained route and report no release.
            assert 0 < len(routes) <= 6
            ...
            for i, route in enumerate(routes):
                ...
                assert all(0 <= state[0] < safe.shape[0] and 0 <= state[1] < safe.shape[1]
                           and 0 <= state[2] < 4 for state in route['states'])
                ...
                if route['asset_index'] is not None:
                    ...
                    target_rear = group['rear_boundary_xy'].copy()
                    associated_count = None
                    # Unchanged V8.1 observed-cell navigation boundary.
                    occupied = np.argwhere(mapper.belief == 1)
                    centers_xy = np.column_stack([(occupied[:, 1] + .5) * c.resolution_m,
                                                  (mapper.shape[0] - occupied[:, 0] - .5) * c.resolution_m])
                    local = ((centers_xy >= group['observed_low'][:2] - c.resolution_m)
                             & (centers_xy <= group['observed_high'][:2] + c.resolution_m)).all(1)
                    associated_count = int(local.sum())
                    # Enumerate four cell corners independently of the
                    # production center-plus-support-function expression.
                    if local.any():
                        offsets = np.array([[-.5, -.5], [-.5, .5], [.5, -.5], [.5, .5]]) * c.resolution_m
                        corners = centers_xy[local, None, :] + offsets[None]
                        furthest = float(np.max(corners @ group['back_axis']))
                        displacement = max(0., furthest - float(target_rear @ group['back_axis']))
                        target_rear += displacement * group['back_axis']
                    error = float(np.linalg.norm(pose[:2, 3] - (target_rear + desired_offset * group['back_axis'])))
                    target_row = next(r for r in audit['target_audit'] if r['role'] == route['group'])
                    # Unchanged V8.1 observed-cell navigation boundary.
                    np.testing.assert_allclose(target_rear, target_row['navigation_rear_boundary_xy'], rtol=0, atol=1e-12)
                    row.update(navigation_rear_boundary_xy=target_rear.tolist(), associated_observed_occupied_cells=associated_count)
                    np.testing.assert_allclose(target_rear + desired_offset * group['back_axis'], target_row['desired_xy'], rtol=0, atol=1e-12)
                    np.testing.assert_allclose(error, target_row['nearest_error_m'], rtol=0, atol=1e-12)
            ...
            score_review = verify_scores_and_actual_remaps(mapper, frames, scans, routes, predictions)
            ...
            reports.append({'context': context_id, 'arrangement': arrangement, 'raw_prefix_frames_exact': 151,
                            ...
                            'score_and_actual_semantic_remap_review': score_review,
                            'optical_exclusivity_and_visit_both_gates_required': False,
        ...
        for family in ('scores', 'rate_scores'):
            for mode in ('N', 'G', 'O', 'M'):
                np.testing.assert_array_equal(first['predictions'][family][mode], second['predictions'][family][mode])
            np.testing.assert_array_equal(first['predictions'][family]['S'], second['predictions'][family]['X'])
            np.testing.assert_array_equal(second['predictions'][family]['S'], first['predictions'][family]['X'])
        ...
        del pair
    for name, digest in choice_seal.items(): assert sha(args.run / name) == digest
    assert freeze_review == verify_rule_freeze(args.freeze, args.snapshot, protocol_path, protocol['context_manifest'])
    ...
    assert len(reports) == 8 and raw_count == 1208 and len(pairs) == 4
    ...
    write(args.output / 'summary.json', {'status': 'passed_independent_release_review', 'release_permitted': structural_pass and extra_pass,
          'release_allowed': structural_pass and extra_pass, 'protocol_path': protocol_path, 'candidate_module': candidate_module,
          ...
          'actual_semantic_intervention_mapper_frames': raw_count * 2,
          'rate_and_total_score_arrays_verified': len(reports) * 12,
          ...
          'rule_freeze_review': freeze_review, 'histories': reports, 'pairs': pairs})


def run(args):
    args.run = args.run.resolve(); args.output = args.output.resolve(); args.freeze = args.freeze.resolve()
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='competition-v9-release-audit-') as directory:
            ...
            with zipfile.ZipFile(args.run / 'sources.zip') as archive:
                ...
                assert len(archive.namelist()) == len(meta['source_sha256'])
            ...
            subprocess.run([sys.executable, str(Path(__file__).resolve()), '--worker', '--snapshot', str(snapshot),
                            '--run', str(args.run), '--output', str(args.output), '--freeze', str(args.freeze)], cwd=snapshot, check=True)
        for name, digest in manifest.items(): assert sha(args.run / name) == digest, name
        assert sha(__file__) == own_sha
        summary = read(args.output / 'summary.json')
        summary.update(run=str(args.run), prepared_manifest_sha256=sha(args.run / 'artifact_hashes.json'),
                       artifact_manifest_sha256=sha(args.run / 'artifact_hashes.json'),
...
if __name__ == '__main__':
    if not __debug__:
        raise RuntimeError('Assertions implement integrity gates; Python optimization mode is not permitted')
    ...
    parser.add_argument('--freeze', type=Path, default=Path('audit_results/competition_v9_rule_freeze_20260911'))
