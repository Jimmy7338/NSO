...
def generate_options(mapper, position, heading, remaining_budget, return_anchor,
    ...
    camera_unknown = ~seen.astype(bool) & (belief != 1)
    occupied = belief == 1
    ...
    coverage = min(pool, key=lambda row: (-row['unknown_cells'] / row['cost'], row['cost'], row['pose']))
    add(coverage, 'coverage_anchor')
    audit['common_role_audit'].append({'role': 'coverage_anchor', 'reason': 'highest_shared_unknown_proxy_per_reserved_cost',
                                     'pose': list(coverage['pose']), 'planned_paid_cost': coverage['cost'],
