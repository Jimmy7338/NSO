"""Prospectively fixed factorial geometry with swapped hidden industrial assets.
...
DEFAULT_CONTEXT_MANIFEST = Path(__file__).resolve().parents[1] / "configs/virtual3d/competition_v9_contexts.json"
...
def _sensor_seed(outer_seed, context_id):
    token = f"nso-competition-v9/{outer_seed}/{context_id}/common-sensor-stream".encode()
    return int.from_bytes(hashlib.sha256(token).digest()[:4], "little")


def load_competition_contexts(path=None):
    """Read the four predeclared factorial parents without constructing any world."""
    manifest = json.loads(Path(path or DEFAULT_CONTEXT_MANIFEST).read_text())
    if manifest["schema_version"] != "competition_v9_contexts/1":
        raise ValueError("unknown competition context contract")
    if [row["context_id"] for row in manifest["contexts"]] != ["Q0", "Q1", "Q2", "Q3"]:
    ...
    for row in manifest["contexts"]:
        if row["outer_seed"] != {"Q0": 951, "Q1": 952, "Q2": 953, "Q3": 954}[row["context_id"]]:
            raise ValueError("the generator opens only the four declared prospective seeds")
...
class CompetitionContextV9:
    ...
    def __post_init__(self):
        dimensions = {"Q0": (1., 1., .01), "Q1": (1., 1.4, .02),
                      "Q2": (1.4, 1., .02), "Q3": (1.4, 1.4, .01)}
        if self.context_id not in dimensions or self.role != "prospective_finite_mechanism_confirmation":
            raise ValueError("only Q0..Q3 prospective factorial parents are declared")
        index = int(self.context_id[-1])
        if self.outer_seed != 951 + index or self.sensor_seed != _sensor_seed(self.outer_seed, self.context_id):
            raise ValueError("invalid parent or common sensor stream")
        if self.mirror_x:
            raise ValueError("V9 uses the unchanged P0 prefix for every parent")
        checks = ((self.object_front_centers_xy_m, ((2.1, 4.3), (6.1, 4.3))),
                  (self.initial_xy_m, (4.1, .7)), (self.final_prefix_xy_m, (4.1, 4.7)))
        if any(not np.allclose(a, b, rtol=0, atol=1e-12) for a, b in checks):
            raise ValueError("context geometry differs from the prospective construction")
        ...
        if (self.object_width_m, self.object_depth_m, self.depth_sigma_m) != dimensions[self.context_id]:
            raise ValueError("dimensions/noise differ from the predeclared factorial design")
...
def get_competition_context(context_id, path=None):
    ...
    return CompetitionContextV9(**data)


@dataclass(frozen=True)
class CompetitionConfigV9(VirtualConfigV2):
    width_m: float = 8.4
    height_m: float = 7.2
    ...
    def __post_init__(self):
        ...
        fixed = dict(width_m=8.4, height_m=7.2, resolution_m=.2, robot_radius_m=.2,
                     ...
                     max_depth_m=4., dropout=.01,
                     laser_height_m=.25, laser_rays=180, voxel_m=.03,
                     truncation_m=.12, pose_noise_m=0., action_duration_s=1.)
        if self.depth_sigma_m not in (.01, .02) or any(getattr(self, key) != value for key, value in fixed.items()):
            raise ValueError("V9 construction fixes geometry, sensor resolution/noise and robot scale")
...
class CompetitionWorldV9(VirtualWorldV2):
    ...
    def __init__(self, context, arrangement="shelf_west", config=None,
        ...
        if not isinstance(context, CompetitionContextV9):
        ...
        self.config = config or CompetitionConfigV9(depth_sigma_m=context.depth_sigma_m)
        if not isinstance(self.config, CompetitionConfigV9):
            raise TypeError("CompetitionConfigV9 required")
        if self.config.depth_sigma_m != context.depth_sigma_m:
            raise ValueError("sensor noise must match the frozen parent")
        ...
        ow, od = context.object_width_m, context.object_depth_m
        half = ow / 2
        common_local = (
            (-half, 0., 0., ow, od, .35),
            (-half, 0., .35, ow, .08, 1.25),
            (-half, .08, .35, .08, od - .08, 1.25),
            (half - .08, .08, .35, .08, od - .08, 1.25),
            (-half + .08, .08, 1.52, ow - .16, od - .08, .08),
        )
        ...
        for (cx, cy), category in zip(context.object_front_centers_xy_m, categories):
            if category == 2:
                for z in (.55, .9, 1.25):
                    add_box(cx - half + .08, cy + .08, z, ow - .16, od - .08, .08, category)
            else:
                # Fill only the cavity and rear opening: no new external front
                # or side triangles can perturb the common prefix first hits.
                add_box(cx - half + .08, cy + .08, .35, ow - .16, od - .16, 1.17, category)
                add_box(cx - half + .08, cy + od - .08, .35, ow - .16, .08, 1.17, category)

        self.mesh, self.triangle_classes, union_audit = union_surface_from_boxes(self._solid_primitives)
        self.competition_truth = dict(generator="paired_competition_v9", context_id=context.context_id,
...
def create_competition_world(context_id, arrangement, config=None,
    ...
    return CompetitionWorldV9(get_competition_context(context_id, manifest_path), arrangement,
