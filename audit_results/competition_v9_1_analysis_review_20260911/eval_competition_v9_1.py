#!/usr/bin/env python3
"""Execute the complete sealed 44-branch prospective finite-domain validation.
...
def run(prepared, review_path, output):
    ...
    if (protocol['schema_version'] != 'competition_v9_1_validation_protocol/1'
            or protocol['parent_order'] != ['Q0', 'Q1', 'Q2', 'Q3']
            or protocol['physical_branch_limit'] != 44):
    ...
    source_names = sorted(set(metadata['source_sha256']) | {'scripts/eval_competition_v9_1.py', 'scripts/replay_competition_v9_1.py', 'scripts/analyze_competition_v9_1.py', 'scripts/analyze_competition_v8_1.py'})
    ...
    write_json(output / 'pre_execution_seal.json', {'decision_assets': seal, 'source_sha256': hashes,
               ...
               'protocol': protocol, 'physical_branch_limit': 44,
    ...
    meta = {'status': 'running', 'prepared': str(prepared.resolve()), 'source_sha256': hashes,
            'scope': 'all 44 prospectively fixed shared validation branches; explicit prior; no training or calibration',
    ...
    try:
        for context in protocol['parent_order']:
            for arrangement in protocol['arrangement_order']:
                ...
                if [r['candidate_id'] for r in routes] != protocol['candidate_ids_by_parent'][context]:
                    raise ValueError('exactly all predeclared shared candidates must be executed')
                for route in routes:
        ...
        if len(outcomes) != 44: raise RuntimeError('finite branch set is incomplete')
        ...
        write_json(output / 'summary.json', {'outcomes': outcomes, 'physical_branches': 44,
                   'paid_branch_actions': sum(r['paid_actions'] for r in outcomes),
                   'shared_prefix_actions_per_history': 150, 'unique_prefix_histories': 8})
        meta.update(status='complete_execution_pending_independent_replay', new_physical_branches=44,
