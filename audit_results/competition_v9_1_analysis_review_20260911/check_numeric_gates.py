...
analyzer_ast = ast.parse((HERE / 'analyze_competition_v9_1.py').read_text())
...
def base():
    ...
    return {'protocol': protocol,
            'rows': [{'collision_count': 0, 'failure': None, 'original_target_reached': True,
                      'returned_to_anchor': True, 'full_original_route_completed': True} for _ in range(44)],
            'metadata': {'new_physical_branches': 44}, 'verification': gate(True),
            ...
            'histories': [{'context': p, 'arrangement': a,
                           'families': {f: {'M_equals_G': True, 'choices': {'S': i + 2}}
                                        for f in ('rate', 'total')}}
...
changed('missing_branch', 'all_44_complete', lambda d: d['rows'].pop())
...
for index, parent in enumerate(parents):
    changed('no_choice_exchange_' + parent, parent + '_semantic_choice_exchange',
            lambda d, i=index: d['histories'][2 * i + 1]['families']['total']['choices'].update(S=2))
small = base()
...
report = {'status': 'passed_pure_synthetic_numeric_gate_review', 'checks': len(results), 'cases': results,
          ...
          'reviewed_analyzer_sha256': hashlib.sha256((HERE / 'analyze_competition_v9_1.py').read_bytes()).hexdigest(),
          'reviewed_protocol_sha256': hashlib.sha256((HERE / 'competition_v9_1_validation_protocol.json').read_bytes()).hexdigest()}
