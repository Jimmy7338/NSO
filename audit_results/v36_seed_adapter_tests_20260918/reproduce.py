...
def main():
    ...
    for name in ('result.json', 'artifact_hashes.json', 'manifest.json', 'sources.zip', 'stdout.txt'):
        p = 'audit_results/v36_seed_adapter_tests_attempt01_20260918/'+name
        inputs[p] = sha(ROOT/p)
    ...
    output = dict(status='passed' if passed else 'failed', exit_code=0 if passed else 1,
        tests_run=result.testsRun, test_duration_seconds=duration, test_execution_count=2,
        prior_attempt='audit_results/v36_seed_adapter_tests_attempt01_20260918',
        prior_failure='Test reference used float64 initial disparity versus the unchanged original function float32 input division; one-ULP difference. Only reference dtype corrected, no adapter/policy/noise change.',
        seed=350918, compared_existing_seed=1901,
