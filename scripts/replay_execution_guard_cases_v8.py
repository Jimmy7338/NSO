#!/usr/bin/env python3
"""Independent replay of the fixed six original-success guard interventions."""
import argparse
...
def worker(args):
    ...
    cases = [('T2','storage_shelves',2),('T2','storage_shelves',5),
    ...
    require(seal['schema_version'] == 'execution_guard_v8_fixed_success_impacts/1', 'wrong fixed experiment')
    require([(c['context_id'],c['family'],c['candidate_id']) for c in seal['cases']] == cases, 'fixed six cases differ')
    for (context_id, family, candidate_id), case in zip(cases, seal['cases']):
        expected_path = f'{context_id}/{family}/candidate_{candidate_id:03d}'
        require(case['path'] == expected_path, 'case path differs from fixed inventory')
        folder = args.run / expected_path
        ...
        branch = folder / f'guarded_candidate_{candidate_id:03d}'; route = read(folder / 'route.json')
        require(route['candidate_id'] == candidate_id, 'wrong original route')
        ...
        at_anchor = tuple(states[-1]) == anchor
        final_safe = distance_transform_edt(np.pad(mapper.belief == 0,1,constant_values=False))[1:-1,1:-1] > config.robot_radius_m/config.resolution_m + np.sqrt(2)/2
        require(r['final_footprint_known_safe'] == bool(final_safe[tuple(states[-1][:2])]), 'final footprint claim')
        if terminal == 'returned_to_prefix_anchor':
            require(at_anchor and not r['collisions'] and r['guarded_return_completed'] and return_paid > 0, 'false successful return')
        elif terminal.startswith('return_unavailable:'):
            plans = [d for d in decisions if d['kind'] in ('return_plan','return_plan_at_anchor','return_replan_after_denial')]
            require(plans and not plans[-1]['available'] and terminal == 'return_unavailable:' + plans[-1]['reason'], 'unexplained stop')
            require(not r['guarded_return_completed'], 'unavailable return counted as success')
        else:
            raise ValueError('unexpected terminal mode requires explicit verification, not implicit success')
        reports.append({'context': context_id, 'family': family, 'candidate_id':candidate_id,
                        'paid_actions': paid, 'decisions': len(decisions), 'terminal_reason':terminal,
                        'at_anchor_including_heading':at_anchor,'independent_BFS_and_gate_passed': True,
                        'raw_sensors_and_mesh_exact': True})
    require(result['paid_actions'] == sum(r['paid_actions'] for r in reports), 'total actions')
    dump(args.result, {'cases': reports, 'paid_actions': result['paid_actions'], 'cases_checked': 6,
                      'returned_cases':sum(r['at_anchor_including_heading'] for r in reports),
                      'stopped_without_return_cases':sum(not r['at_anchor_including_heading'] for r in reports)})
...
def main(args):
    ...
    try:
        ...
        report.update(status='passed_full',passed_full=True,artifact_manifest_sha256=sha(args.run/'artifact_hashes.json'),
                      ...
                      scope='six affected originally successful routes only; independent gate/BFS, reused archived mapper/physical simulator; replay success does not mean every task completed')
