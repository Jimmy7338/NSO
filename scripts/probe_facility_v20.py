#!/usr/bin/env python3
"""Bounded N/G online coverage probe with frozen sources and fresh replays.

Preparation is explicit, uses existing V19 reference files read only, and does
not run any paid action. The default batch contains exactly four N trajectories.
G requires an explicitly prepared separate output directory. No V19 first-option
intervention or semantic training is installed by this collector.
"""
import os
...
CONFIG = ROOT / 'configs/virtual3d/facility_coverage_v20_probe.json'
...
def validate_config(config):
    ...
    if config['training_allowed'] or config['automatic_mode_expansion']:
        raise ValueError('No training or automatic G batch is allowed')


def bootstrap(case, config):
    # Lazy import permits syntax/CLI inspection while runtime development proceeds.
    from nso.facility_runtime_v20 import facility_components_v20, FacilityRuntimeV20
    ...
    components = facility_components_v20(args, world.shape)
    runtime = FacilityRuntimeV20(components, 1, world.shape)
...
def run_case(root, index, replay=False):
    ...
    if not state['closed']:
        runtime.close_sensor_episode(0, reason='coverage_probe_terminated')
    after = evaluate()
    mesh = mapper.mesh()
    ...
    result = json_value(dict(case=case, actions=actions, before=before, after=after,
        ...
        task='six_facility_common_online_coverage_probe',
...
def main():
    ...
    parser.add_argument('--mode', choices=('N', 'G'), help='Preparation only; default N')
