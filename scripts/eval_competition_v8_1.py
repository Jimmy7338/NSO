#!/usr/bin/env python3
"""Execute the complete sealed 24-branch development positive control.
...
from env.virtual3d_competition_v8 import create_competition_world
...
def run(prepared, review_path, output):
    ...
    if metadata['status'] != 'complete_structural_pass' or not read(prepared / 'structure_summary.json')['passed']:
        raise ValueError('all four structural histories must pass before any branch')
    ...
    source_names = sorted(set(metadata['source_sha256']) | {'scripts/eval_competition_v8_1.py'})
    ...
    write_json(output / 'pre_execution_seal.json', {'decision_assets': seal, 'source_sha256': hashes,
               ...
               'protocol': protocol, 'physical_branch_limit': 24,
    ...
    meta = {'status': 'running', 'prepared': str(prepared.resolve()), 'source_sha256': hashes,
            'scope': 'all 24 shared development branches; explicit prior; no training or calibration',
    ...
    try:
        ...
        if len(outcomes) != 24: raise RuntimeError('finite branch set is incomplete')
        ...
        write_json(output / 'summary.json', {'outcomes': outcomes, 'physical_branches': 24,
                   'paid_branch_actions': sum(r['paid_actions'] for r in outcomes),
                   'shared_prefix_actions_per_history': 150, 'unique_prefix_histories': 4})
        meta.update(status='complete_execution_pending_independent_replay', new_physical_branches=24,
