...
from nso.cpu_four_modules_v16_2 import staged_components_v16_2 as axis_components_v16
...
from nso.hierarchical_options_v16 import generate_options as base_options
...
def check_checkpoint(comp, mapper, packet, original):
    ...
    comp.args.cpu_max_candidates = 5 + 4 * len(state['assets'])
    ...
    base, _ = base_options(AxisHistoryViewV16(mapper, state['axis_frames']), packet.position, packet.heading,
        state['ledger'].remaining_budget, state['return_anchor'],
        max_candidates=candidate_capacity(len(state['assets'])), coverage_strategy='total_diverse',
        coverage_slots=4, attempted_camera_mask=attempted)
