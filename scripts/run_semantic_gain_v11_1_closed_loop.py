...
def main(prepared, output, smoke=False):
    ...
    protocol_path = ROOT / "configs/virtual3d/semantic_gain_v11_1_closed_loop_development_protocol.json"
    ...
    manifest = dict(schema_version="semantic_gain_v11_1_closed_loop/1", status="running",
    ...
    conditions = ("learned_semantic_feedback", "learned_semantic_no_feedback",
                  "learned_geometry", "fixed_semantic") if smoke else tuple(protocol["conditions"])
    contexts = ("Q2",) if smoke else tuple(protocol["contexts"])
    arrangements = tuple(protocol["arrangements"]) if smoke else tuple(protocol["arrangements"])
    rows = []
            ...
            low_confidence_matches_geometry=all(
                a["actions"] == b["actions"] for a, b in zip(
                    ...
                    sorted([r for r in rows if r["condition"] == "learned_geometry"],
                           key=lambda x:(x["context"],x["arrangement"])))),
            feedback_non_degrading=(comparisons[
                "semantic_vs_learned_semantic_no_feedback"]["mean_difference"] >= -1e-12))
    summary = dict(schema_version="semantic_gain_v11_1_closed_loop_result/1",
