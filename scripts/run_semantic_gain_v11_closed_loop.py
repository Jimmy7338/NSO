#!/usr/bin/env python3
"""Run the frozen V11 learned-gain head through the four-module CPU loop."""
import os
...
def execute(prepared, model, context, arrangement, condition):
    ...
    args = SimpleNamespace(nso_backend="cpu_v10", eval=True, train_global=False,
        ...
        cpu_max_candidates=6, cpu_coverage_slots=4, cpu_planner_revision="v11",
        ...
        run_id=f"v11-{context}-{arrangement}-{condition}")
...
def pair(rows, primary, baseline, metric="inspection_joint"):
...
def main(prepared, output, smoke=False):
    ...
    protocol_path = ROOT / "configs/virtual3d/semantic_gain_v11_closed_loop_development_protocol.json"
    ...
    manifest = dict(schema_version="semantic_gain_v11_closed_loop/1", status="running",
        created_utc=datetime.now(timezone.utc).isoformat(), protocol=protocol, smoke=bool(smoke),
        source_sha256={str(p.relative_to(ROOT)): sha(p) for p in sources},
        worlds_constructed_before_source_freeze=0, independent_confirmation=False)
    write(output / "manifest.json", manifest)
    conditions = ("learned_semantic_feedback", "learned_semantic_no_feedback",
                  "learned_geometry") if smoke else tuple(protocol["conditions"])
    contexts = ("Q0",) if smoke else tuple(protocol["contexts"])
    arrangements = ("shelf_west",) if smoke else tuple(protocol["arrangements"])
    rows = []
    for condition in conditions:
        for context in contexts:
            for arrangement in arrangements:
                ...
                print(condition, context, arrangement, row["inspection_joint"],
                      row["conditional_gain"]["observed_actions"], flush=True)
    by = {condition: dict(mean_inspection_joint=float(np.mean([r["inspection_joint"]
            ...
            low_confidence_matches_geometry=all(
                a["actions"] == b["actions"] for a, b in zip(
                    ...
                    sorted([r for r in rows if r["condition"] == "learned_geometry"],
                           key=lambda x:(x["context"],x["arrangement"])))),
            feedback_changes_action_and_improves_semantic=(comparisons[
                "semantic_vs_learned_semantic_no_feedback"]["mean_difference"] > 0
                and comparisons["semantic_vs_learned_semantic_no_feedback"]["ties"] < 8))
    summary = dict(schema_version="semantic_gain_v11_closed_loop_result/1",
        status="passed" if all(gates.values()) else "failed", development_only=True,
        branches=len(rows), by_condition=by, comparisons=comparisons,
        gates=gates, passed=all(gates.values()), smoke=bool(smoke))
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    try:
        main(a.prepared.resolve(), a.output.resolve(), a.smoke)
