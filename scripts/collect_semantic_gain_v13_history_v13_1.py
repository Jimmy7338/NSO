#!/usr/bin/env python3
"""V13.1 history collector: terminal-budget events are not planning decisions."""
import os
...
def main(output, protocol):
    ...
    manifest = dict(status="running", schema_version="semantic_gain_v13_history/1.1",
    ...
    try:
        ...
        while not runtime.states[0]["closed"]:
            ...
            if (not state["active_actions"] and state["phase"] != "return"
                    and runtime.components._cpu_backend.scenes[0]["ledger"].remaining_budget > 0):
                ordinal += 1
            ...
            if checkpoint is not None:
                ...
                remaining = runtime.components._cpu_backend.scenes[0]["ledger"].remaining_budget
                if any(not 0 < c["cost"] <= remaining for c in checkpoint["candidates"]):
                    raise ValueError("candidate cost exceeds the actual decision budget")
