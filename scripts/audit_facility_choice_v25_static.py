...
import numpy as np
from env.facility_choice_v25 import (FacilityChoiceWorldV25,StaticGeometryErrorV25,
...
OUTPUT=ROOT/'audit_results/facility_choice_v25_static_r0_20260915'
PROTOCOL=ROOT/'docs/research/V25_PARTITION_STATIC_PROTOCOL_20260915.md'
...
def execute(output,deadline,progress):
    ...
    require(all(sha(ROOT/k)==v for k,v in protected.items()),'Old source changed')
    paths=[Path(__file__).resolve(),PROTOCOL,ROOT/'env/facility_choice_v25.py']
    ...
    result=dict(status='complete_static_only',world_version=VERSION_V25,parents=parents,cases=cases,
        ...
        automatic_followup_trajectories_authorized=False,repair_count=0)
