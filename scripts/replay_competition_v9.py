#!/usr/bin/env python3
"""Independently replay the complete sealed V9 48-branch prospective run.
...
def worker(args):
    ...
    compare(protocol, read(args.snapshot / 'configs/virtual3d/competition_v9_validation_protocol.json'), 'archived protocol equals execution seal')
    ...
    require(protocol['schema_version'] == 'competition_v9_validation_protocol/1', 'V9 protocol required')
    ...
    require(protocol['reference']['samples_requested'] == 32000 and seal['physical_branch_limit'] == 48, 'reference/branch limits')
    ...
    for context_id in protocol['parent_order']:
        ...
        for arrangement in protocol['arrangement_order']:
            ...
            for method, scores in prediction['rate_scores'].items():
                require(len(scores) == 6 and np.isfinite(scores).all(), 'invalid rate score vector')
                selected = max(range(6), key=lambda i: (scores[i], -routes[i]['cost'], -i))
    ...
    compare(summary, {'outcomes': all_outcomes, 'physical_branches': 48,
    ...
    require(meta['new_physical_branches'] == 48 and meta['paid_actions'] == sum(r['paid_actions'] for r in reports)
    ...
    write(args.result, {'branches': reports, 'physical_branches': 48, 'branches_checked': 48, 'branches_total': 48,
...
def main(args):
    ...
    report = {'status': 'failed', 'passed_full': False, 'partial': False, 'max_branches': None,
              'run': str(args.run), 'scope': 'all 48 shared prospectively declared V9 branches; independent raw replay and accounting only'}
    try:
    ...
    if target.exists():
        ...
        retained = root / 'audit_results' / 'competition_v9_replay_attempts' / sha(target)
