#!/usr/bin/env python3
"""Reversible storage compaction for the fixed, fully replayed V9.1 run.
...
AUDIT = ROOT / 'audit_results/competition_v9_1_mesh_compaction_20260911'
RUNS = {'competition_v9_1_execution_20260911': (44, 88, 'competition')}
REMOVABLE = {f'{parent}/{arrangement}/candidate_{candidate:03d}/{stage}_mesh.npz'
             for parent in ('Q0', 'Q1', 'Q2', 'Q3')
             for arrangement in ('shelf_west', 'shelf_east')
             for candidate in range(4 if parent == 'Q3' else 6)
             for stage in ('arrival', 'final')}
METRIC_AUDIT = ROOT / 'audit_results/competition_v9_1_independent_metrics_20260911'
...
def targets(manifest, kind):
    ...
    for name in manifest:
        ...
        if kind == 'inspection':
            ...
            require(p.parts[0] in ('Q0', 'Q1', 'Q2', 'Q3') and p.parts[1] in ('shelf_west', 'shelf_east'), 'unexpected context')
        result.append(name)
    require(set(result) == REMOVABLE, 'fixed Q0/Q1/Q2 six-route and Q3 four-route inventory changed')
...
def bindings(run):
    ...
    require(metadata['status'] == 'complete_execution_pending_independent_replay'
            and metadata['new_physical_branches'] == 44 and metadata['failures'] == 0,
            'only the complete failure-free V9.1 run is compactable')
    metric_manifest = read(METRIC_AUDIT / 'artifact_hashes.json')
    for name, digest in metric_manifest.items():
        require(sha(relative(METRIC_AUDIT, name)) == digest, 'independent metric audit changed')
    metric = read(METRIC_AUDIT / 'verification.json')
    metric_inputs = read(METRIC_AUDIT / 'input_hashes.json')
    require(metric['status'] == 'passed_independent_metric_arithmetic_and_complete_pool_audit'
            and metric['run'] == str(run) and metric['branches'] == 44
            and metric['numeric_checks'] == 3479 and metric['all_progression_decisions_match'],
            'complete independent metric audit required')
    require(metric_inputs[str(run / 'artifact_hashes.json')] == hashes['artifact_hashes.json']
            and metric_inputs[str(run / 'verification.json')] == hashes['verification.json'],
            'metric audit refers to other physical evidence')
    hashes['independent_metric_audit_manifest'] = sha(METRIC_AUDIT / 'artifact_hashes.json')
    hashes['independent_metric_verification'] = sha(METRIC_AUDIT / 'verification.json')
...
def worker(args):
    ...
    if kind == 'inspection':
        ...
        from env.virtual3d_competition_v9 import CompetitionConfigV9 as Config
