#!/usr/bin/env python3
"""Seal all eight measured prefixes, shared choices, and structural gates.
...
from nso.competition_objective_v9 import score_routes
...
def check_pair(first, second):
    ...
    for family in ('scores', 'rate_scores'):
        for name in ('N', 'G', 'O', 'M'):
            np.testing.assert_array_equal(first['predictions'][family][name], second['predictions'][family][name])
...
def prepare(output):
    ...
    protocol_path = ROOT / 'configs/virtual3d/competition_v9_validation_protocol.json'
    protocol = read(protocol_path)
    freeze = ROOT / 'audit_results/competition_v9_rule_freeze_20260911'
    receipt = read(freeze / 'receipt.json')
    for name, digest in read(freeze / 'artifact_hashes.json').items():
        if file_hash(freeze / name) != digest: raise RuntimeError('rule-freeze artifact changed')
    for name, digest in receipt['source_sha256'].items():
        if file_hash(ROOT / name) != digest: raise RuntimeError('preconstruction rule changed')
    ...
    names = sorted({str(p.relative_to(ROOT)) for folder in ('env', 'nso', 'utils')
        ...
        'scripts/prepare_competition_v9.py', 'scripts/prepare_response_v7.py',
        'scripts/eval_counterfactual_views.py', 'configs/virtual3d/competition_v9_contexts.json',
        'configs/virtual3d/competition_v9_validation_protocol.json', 'requirements-3d.lock.txt'})
    ...
    shutil.copyfile(freeze / 'receipt.json', output / 'rule_freeze_receipt.json')
    metadata = {'status': 'running_prefix_preparation', 'source_sha256': hashes,
                'protocol_path': str(protocol_path.relative_to(ROOT)),
                'rule_freeze_path': str(freeze), 'rule_freeze_manifest_sha256': file_hash(freeze / 'artifact_hashes.json'),
                'new_candidate_branches': 0, 'role': 'prospective_finite_mechanism_confirmation',
        ...
        for context_id in protocol['parent_order']:
            ...
            for arrangement in COMPETITION_ARRANGEMENTS:
                ...
                try:
                    ...
                    write_json(folder / 'failed_prefix.json', {'step': world.step_count,
                        'position': list(world.position), 'heading': world.heading,
                        'collisions': world.collisions, 'error': str(exc)})
                    raise
                if not np.allclose(raw[-1]['frame'].world_from_camera[:2, 3], world.context.final_prefix_xy_m,
                                   rtol=0, atol=1e-12) or world.heading != world.context.final_prefix_heading:
                    raise RuntimeError('fixed prefix endpoint differs')
                ...
                predictions = score_routes(mapper, routes, prefix_poses) if routes else {'scores': {m: [] for m in ('G','O','N','S','X','M')}, 'rate_scores': {m: [] for m in ('G','O','N','S','X','M')}, 'selected_candidate_ids': {}, 'rate_selected_candidate_ids': {}}
                ...
                if routes:
                    for mode, expected in (('shuffled', 'X'), ('absent', 'G')):
                        ...
                        for family in ('scores', 'rate_scores'):
                            np.testing.assert_array_equal(actual[family]['S'], predictions[family][expected])
                        del changed
                ...
                summary = {'context': context_id, 'arrangement': arrangement, 'prefix_paid_actions': 150,
                           ...
                           'choices': predictions['selected_candidate_ids'],
                           'rate_choices': predictions.get('rate_selected_candidate_ids', {}), 'geometry_sha256': geom,
