#!/usr/bin/env python3
"""Fixed observed-map guard regressions with sealed per-candidate case paths.
...
CASE_IDS = (("T2", "storage_shelves", 2), ("T2", "storage_shelves", 5),
            ("T2", "ventilation_baffles", 2), ("T2", "ventilation_baffles", 5),
            ("T5", "storage_shelves", 5), ("T5", "ventilation_baffles", 5))
CASES = tuple({"context_id": context, "family": family, "candidate_id": candidate,
               "path": f"{context}/{family}/candidate_{candidate:03d}"}
              for context, family, candidate in CASE_IDS)
BUDGET = 48
...
def worker(args):
    ...
    require(seal["cases"] == list(CASES), "fixed six-case list changed")
    require(seal["protocol"]["branch_budget"] == BUDGET, "sealed budget changed")
    for name, expected in seal["decision_assets"].items():
        require(sha(output / name) == expected, f"sealed decision input differs: {name}")
    summaries = []
    # Case paths and candidate IDs are read from the seal, never outcomes.
    for case in seal["cases"]:
        context_id, family, candidate_id = case["context_id"], case["family"], case["candidate_id"]
        folder = (output / case["path"]).resolve()
        require(folder.is_relative_to(output), "unsafe sealed case path")
        ...
        require(route["candidate_id"] == candidate_id and tuple(route["states"][0]) == anchor
        ...
        initial_safe = guard.safe_grid(mapper.belief)
        require(all(initial_safe[tuple(state[:2])] for state in route["states"]),
                "original sealed candidate leaves the complete known-safe footprint grid")
        result_dir = folder / f"guarded_candidate_{candidate_id:03d}"
        ...
        result = {"context_id": context_id, "family": family, "candidate_id": candidate_id, "case_path": case["path"],
                  ...
                  "scope": "six originally successful routes fixed by the all-96 latest-map audit; no system safety or semantic efficacy conclusion"}
        dump(result_dir / "result.json", result); summaries.append(result)
        print("guard diagnostic", context_id, family, candidate_id, "paid", len(actions), "terminal", terminal, flush=True)
        del mapper, world
    ...
    dump(output / "results.json", {"cases": summaries, "paid_actions": sum(r["paid_actions"] for r in summaries),
         "max_paid_actions": len(seal["cases"]) * BUDGET, "dependencies": {"numpy": np.__version__, "open3d": open3d.__version__, "scipy": scipy.__version__},
...
def run(args):
    ...
    require(shutil.disk_usage(output.parent).free >= 400 * 1024**2,
            "insufficient space for the fixed 400 MiB storage reserve")
    ...
    inputs = {}; base_sources = None; contents = None; source_runs = {}
    evidence_path = ROOT / "eval_results/response_v7_all_routes_latest_map_review_20260911/summary.json"
    evidence = read(evidence_path)
    require(evidence["status"] == "passed_all_96_recorded_routes_latest_map_review", "selection audit incomplete")
    expected_cases = sorted((r["context_id"], r["family"], r["candidate_id"]) for r in evidence["rows"]
                            if r["original_successful_round_trip"] and r["guard_would_change_original_route"])
    require(expected_cases == sorted(CASE_IDS), "the fixed six cases omit or add an original-success guard impact")
    selection_evidence = {"path": str(evidence_path), "sha256": sha(evidence_path),
                          "selection_rule": "all originally successful branches changed in the full 96-route map-only audit"}
    for case in CASES:
        context_id, family, candidate_id = case["context_id"], case["family"], case["candidate_id"]
        ...
        require(sha(source_run / "metadata.json") == old_manifest["metadata.json"], "original metadata changed")
        require(sha(source_run / "sources.zip") == old_manifest["sources.zip"], "original source archive changed")
        source_runs[context_id] = {"path": str(source_run),
            **{name + "_sha256": sha(source_run / name) for name in ("metadata.json", "artifact_hashes.json", "sources.zip")}}
        ...
        dump(folder / "route.json", routes[candidate_id])
    overlays = ("nso/execution_guard_v8.py", "scripts/eval_execution_guard_cases_v8.py", "tests/virtual3d/test_execution_guard_v8.py")
    ...
    decision_assets = {str(p.relative_to(output)): sha(p)
                       for case in CASES for p in (output / case["path"]).rglob("*") if p.is_file()}
    seal = {"schema_version": "execution_guard_v8_fixed_success_impacts/1", "sealed_utc": datetime.now(timezone.utc).isoformat(),
            ...
            "selection_evidence": selection_evidence, "source_runs": source_runs,
            ...
            "zero_outcome_selection": "all six originally successful routes changed in the complete 96-route map audit: T2 both-family candidate2/5 and T5 both-family candidate5; fixed before this execution; no old outcome value read"}}
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="execution-guard-v8-") as temporary:
            ...
            command = [sys.executable, str(snapshot / "scripts/eval_execution_guard_cases_v8.py"), "--worker",
        ...
        require(sha(evidence_path) == selection_evidence["sha256"], "selection evidence changed during execution")
        for source in source_runs.values():
            for name in ("metadata.json", "artifact_hashes.json", "sources.zip"):
                require(sha(Path(source["path"]) / name) == source[name + "_sha256"], "original source binding changed")
        for name in overlays:
            require(sha(ROOT / name) == source_hashes[name], "guard/runner source changed during diagnostic")
        result = read(output / "results.json")
        require(len(result["cases"]) == len(CASES) and result["paid_actions"] <= len(CASES) * BUDGET, "fixed diagnostic matrix exceeded")
        metadata.update(status="complete_execution_pending_independent_replay", completed_cases=len(CASES),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "eval_results/execution_guard_v8_success_impacts_20260911")
