#!/usr/bin/env python3
"""Preregistered V35 online routes: prepare, main, independent replay, analyze.
...
OUTPUT = ROOT / 'audit_results/v35_online_development_20260918'
...
def prepare(review):
    ...
    if not PROTOCOL.is_file(): raise ValueError('online development protocol must exist before preparation')
    ...
    import numpy as np
    scene = read(SCENE); parent = next(p for p in scene['parents'] if p['id'] == 'P00')
    models, prefix = load_public_models_v35(ROOT, 'P00')
    templates = PublicTemplatesV35.from_saved(INFORMATION, 'P00', models[0].poses)
    forecast = [templates.template_information(n) for n in range(len(models[0].poses))]
    static_audit = public_graph_audit(parent, models, prefix)
    ...
    for h in (0, 1):
        for mode in MODES:
            index = len(physical)
            physical.append(dict(index=index, parent='P00', hypothesis=h, mode=mode,
                episode_id=f'v35-online-P00-case{index:02d}'))
    cells = [dict(hypothesis=c['hypothesis'], mode=c['mode'], physical_case=c['index'],
        alias=False, acquisition_independent=True, independent_scene_sample=False) for c in physical]
    config = dict(version='v35-online-development-1', acquisition=acquisition,
        cells=cells, physical_cases=physical,
        ...
        control_scope='online CPU four-module two-template controller with public safe graph',
    ...
    try:
        ...
        write(OUTPUT / 'public_graph_audit.json', static_audit)
        write(OUTPUT / 'public_forecast.json', forecast)
        seeds = [Path(__file__), SCENE, PROTOCOL, OUTPUT / 'config.json',
            OUTPUT / 'public_graph_audit.json', OUTPUT / 'public_forecast.json',
            ROOT / 'scripts/analyze_semantic_chain_v35.py',
        ...
        gates = (INFORMATION, review, MEASUREMENT, SENSOR, ONLINE_GATE, ONLINE_REVIEW, *IMPLEMENTATION_GATES)
...
def require_previous_cases(config, index):
    ...
    if HISTORICAL_MAIN + len(directories) + 1 > MAIN_LIMIT:
        raise ValueError('historical 19/36 main quota exceeded')
...
def paired_prefix_trace_check(trace):
    checked = []
...
def run_case(index, replay=False):
    ...
    try:
        ...
        controller = CPUFourModuleControllerV35(models, templates, prefix, mode=case['mode'],
            total_budget=BUDGET, informative_nodes=config['acquisition']['informative_nodes'])
        ...
        counts['worlds'] = 1
        if (world.noise_seed != config['acquisition']['noise_seed'] or world.config.voxel_m != .04
                or world.config.max_depth_m != 4. or list(world.shape) != config['acquisition']['raster_shape']
        ...
        for paid in range(BUDGET+1):
            ...
            try:
                ...
                pose = observed_pose(packet, config['acquisition']['translation'])
        ...
        if replay:
            ...
            prefix_pairing = paired_prefix_trace_check(trace)
...
def analyze():
    ...
    pairings = []
    for mode in MODES:
        cells = [next(c for c in config['cells'] if c['hypothesis'] == h and c['mode'] == mode) for h in (0, 1)]
        ...
        pairings.append(dict(mode=mode, physical_cases=[c['physical_case'] for c in cells],
    ...
    if not incomplete:
        for threshold in ('02cm', '05cm', '10cm'):
            values = {(c['hypothesis'],c['mode']): by_index[c['physical_case']]['stages']['final']['measurement'][threshold]
                for c in config['cells']}
            row = {'mean_joint': {mode: sum(values[h,mode]['joint'] for h in (0,1))/2 for mode in MODES}}
            for name, treatment, baseline in (('S_vs_G','S','G'), ('S_vs_swapped','S','swapped'),
                    ('swapped_vs_swapped_no_feedback','swapped','swapped_no_feedback')):
                deltas = [values[h,treatment]['joint']-values[h,baseline]['joint'] for h in (0,1)]
                average = sum(deltas)/2; base = row['mean_joint'][baseline]
                row[name] = dict(treatment=treatment, baseline=baseline,
                    paired_joint_differences=deltas, paired_mean_joint_difference=average,
                    relative_mean_joint_difference=average/base if base else None)
            comparisons[threshold] = row
    ...
    semantic_positive = bool(comparisons and comparisons['05cm']['S_vs_G']['paired_mean_joint_difference'] > 0
        and all(comparisons[t]['S_vs_G']['paired_mean_joint_difference'] >= 0 for t in ('02cm','10cm')))
    correct_class_positive = bool(comparisons and comparisons['05cm']['S_vs_swapped']['paired_mean_joint_difference'] > 0)
    feedback_positive = bool(comparisons and comparisons['05cm']['swapped_vs_swapped_no_feedback']['paired_mean_joint_difference'] > 0)
    result = dict(status='stopped' if stopped else 'complete', table=table,
        ...
        measured_online_semantic_score_gate_passed=prerequisites and semantic_positive and correct_class_positive,
        measured_feedback_score_gate_passed=prerequisites and feedback_positive,
        overall_online_development_gate_passed=None,
        overall_gate_pending='separate preregistered same-state posterior-to-action audit; at most two additional saved-state DP calls',
        ...
        route_aliasing_used=False, independent_scene_sample_count=1,
    ...
    print(json.dumps(dict(status=result['status'], new_main_tasks=result['new_main_tasks'],
        measured_online_semantic_score_gate_passed=result['measured_online_semantic_score_gate_passed'],
        measured_feedback_score_gate_passed=result['measured_feedback_score_gate_passed'],
        overall_online_development_gate_passed=None)), flush=True)
