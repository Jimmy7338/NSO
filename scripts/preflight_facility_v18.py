...
def sweep(world):
    ...
    for target in targets:
        # Axis-aligned prescribed tour, all corners independent of class/score.
        while world.position!=target:
            r,c=world.position;dr,dc=target[0]-r,target[1]-c
            heading=(0 if dr<0 else 2) if dr else (1 if dc>0 else 3)
            action='forward' if world.heading==heading else 'right'
            frame,collision,done=world.step(action);actions.append(action)
            if collision or done:raise ValueError('prescribed budget witness infeasible')
    ...
    return dict(actions=actions,paid_actions=len(actions),coverage_2d=coverage,returned=True,
        purpose='GT layout-only feasibility witness; not used by planner or as initial prefix',
...
def main():
    ...
    for name in ('scripts/preflight_facility_v18.py','scripts/collect_semantic_gain_v13_history.py','tests/virtual3d/test_facility_v18.py','configs/virtual3d/facility_documentation_v18_design.json'):
