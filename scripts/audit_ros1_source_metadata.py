#!/usr/bin/env python3
"""Cross-check recording/2 source metadata and payloads without ROS access."""
import argparse
import hashlib
import json
import math
...
def check_metadata_sequence(source):
    ...
    if manifest['schema_version']!='ros1_rgbd_recording/2':raise ValueError('recording/2 metadata required')
    ...
    for sub,suffix in [('frames','.npz'),('scans','.npz'),('metadata','.json')]:
    ...
    for stem in sorted(names):
        ...
        try:
            ...
            if meta['schema_version']!='ros1_rgbd_source_metadata/1':raise ValueError('source metadata schema differs')
            ...
            expected_payloads={f'frames/{stem}.npz',f'scans/{stem}.npz'}
            ...
            if (integer(info['height'],'height',1),integer(info['width'],'width',1))!=frame.depth_m.shape:
                raise ValueError('CameraInfo dimensions differ from depth payload')
            matrices={}
            ...
            intrinsic=matrices['P'][:,:3] if matrices['P'][0,0]>0 else matrices['K']
            if not np.array_equal(frame.intrinsic,intrinsic):raise ValueError('stored intrinsic differs from recorder P/K selection')
            ...
            if laser['motion_compensation_applied'] is not False:raise ValueError('recorder/2 cannot claim deskew')
            if meta['calibration_correctness_verified'] is not False:raise ValueError('recorder/2 cannot certify calibration')
    ...
    return dict(status='failed' if errors else 'passed',schema='ros1_source_metadata_crosscheck/1',
        ...
        unchecked=['actual TF response correctness and sensor calibration','RGB/depth physical registration',
            'CameraInfo binning/ROI omitted by recording/2','original raw depth/range conversion and invalid-ray handling',
        ...
        boundary='Checks saved declarations against saved arrays; does not authenticate sensor calibration, raw bag or real-world accuracy.')
