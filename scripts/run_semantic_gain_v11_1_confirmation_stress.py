...
def prefix_packets(prepared, context, arrangement, world, transform):
    """Bind a variable-length sealed confirmation prefix to exact packets."""
    ...
    if len(generated) != len(records) or len(records) != world.context.prefix_step + 1:
        raise RuntimeError("unexpected sealed confirmation prefix length")
    packets = []
    for index, row in enumerate(records):
        frame = RGBDFrame.load(folder / "frames" / f"{index:04d}.npz")
        scan = PlanarScan.load(folder / "scans" / f"{index:04d}.npz")
        physical = generated[index]
        if (physical["step"] != row["step"]
                or physical["action"] != (None if index == 0 else row["action"])
            ...
            raise RuntimeError(f"physical prefix differs from seal at {index}")
        packets.append(SensorPacket(f"{context}_{arrangement}",
            f"v11_1_confirmation-{context}-{arrangement}", f"frame-{index}", index,
            frame, scan, tuple(map(int, row["position"])), int(row["heading"]),
            "frozen_confirmation_rgbd_and_planar_scan", "simulator_exact_discrete_odometry",
            None if index == 0 else row["action"], False, False).validate(
