#!/usr/bin/env python3
"""V36 figure layout repair only; same sealed values, signed deltas and zero-based axes."""
import argparse
import io
import os
...
OUTPUT = ROOT/'audit_results/v36_confirmation_figures_layout_20260918'
ORIGINAL = ROOT/'audit_results/v36_confirmation_figures_20260918'
PARENTS = ('P00', 'P01')
MODES = ('G', 'S')
...
def run():
    ...
    original_result = read(ORIGINAL/'result.json')
    for relative, expected in original_result['figures'].items():
        if sha(ROOT/relative) != expected:
            raise ValueError('original figure changed: '+relative)
        inputs[relative] = expected
    inputs['scripts/plot_online_confirmation_v36.py'] = sha(ROOT/'scripts/plot_online_confirmation_v36.py')
    ...
    if rows != read(ORIGINAL/'plot_data.json')['rows']:
        raise ValueError('layout revision must preserve every plotted source value')
    ...
    if complete:
        ...
        def save(fig, name):
            for extension in ('png', 'svg'):
                path = ROOT/'docs/research/figures'/f'v36_{name}_20260918_layout.{extension}'
                buffer = io.BytesIO()
        ...
        figure.subplots_adjust(left=.08, right=.99, bottom=.18, top=.77, wspace=.12)
        ...
        figure.legend(handles, names, loc='upper center', bbox_to_anchor=(.5, .925), ncols=2, frameon=False)
    ...
    write(OUTPUT, OUTPUT/'result.json', dict(
        ...
        layout_only=True, plotted_values_equal_original=True,
        original_result_sha256=sha(ORIGINAL/'result.json'),
        source_result_sha256=sha(BATCH/'result.json'),
        source_final_seal_sha256=sha(BATCH/'final_seal.json'),
