"""Common coverage-budget hierarchy; paid sensing and execution stay guarded."""
...
from nso.coverage_budget_v20 import ObservedCoverageLedgerV20, scan_hit_mask
from utils.grid_geometry import visible_mask


class FacilityBackendV20(FacilityBackendV19):
    def __init__(self, args, num_scenes, shape):
        if getattr(args, 'cpu_score_mode', None) not in ('N', 'G'):
            raise ValueError('V20 common capacity probe supports N and G only')
        CPUFourModulesV16.__init__(self, args, num_scenes, shape)
        self.capabilities['v20'] = 'common observed coverage budget; semantic efficacy untested'

    def start_scene(self, scene_idx, **kwargs):
        super().start_scene(scene_idx, **kwargs)
        s = self.scenes[scene_idx]; m = s['mapper']; p = s['packet']
        ledger = ObservedCoverageLedgerV20(m.shape, s['return_anchor'][:2],
        ...
        s['coverage_v20'] = ledger
        s['v20_pending_prediction'] = None
        self._record(scene_idx, 'IGCR', 'coverage_bootstrap_v20',
            dict(task_shape=m.shape, target=.8), ledger.snapshot())
    ...
    def assess_action(self, scene_idx, action):
        ...
        if result['allowed']:
            ...
            s['v20_pending_prediction'] = visible_mask(m.belief == 1,
    ...
    def compute_reward(self, scene_idx):
        ...
        intent = bool(option and option.get('v20_coverage_intent', option['group'].startswith('coverage_')))
        event = s['coverage_v20'].observe(m.belief, action_id=p.action_id,
            coverage_intent=intent, predicted_mask=s.pop('v20_pending_prediction', None),
            radar_hits=scan_hit_mask(p.scan, m.shape, m.config.resolution_m))
        self._record(scene_idx, 'IGCR', 'observe_unique_coverage_v20',
            dict(action_id=p.action_id, coverage_intent=intent), event)
        return value, {**parts, 'coverage_v20': s['coverage_v20'].snapshot()}
    ...
    def select_target(self, scene_idx):
        ...
        if s['graph_version'] != m.frames or s['semantic_version'] != m.frames:
            raise RuntimeError('V20 planning requires current four-module observations')
        if s.get('region_continuation') is not None:
            raise RuntimeError('V20 capacity probe does not install two-stage semantic interventions')
        ...
        for route, row in zip(routes, rows):
            ...
            budget = s['coverage_v20'].assess_route(route, mask, s['ledger'].remaining_budget)
            ...
            row['v20_coverage'] = dict(unique_predicted_unknown_cells=count,
        ...
        admitted = [i for i in range(len(routes)) if rows[i]['v20_coverage']['coverage_budget']['allowed']
                    and scores['G'][i] > 0]
        ...
        if s['selected'] is not None:
            s['selected'].update(option_id=f'{s["scene_id"]}/{s["episode_id"]}/plan-{s["plans"]}',
                ...
                v20_coverage_intent=pressure or routes[index]['group'].startswith('coverage_'),
        ...
        result = dict(selected=s['selected'], candidates=routes, scores=scores, score_audit=rows,
            ...
            quality_budget_admitted_candidates=admitted, coverage_state=s['coverage_v20'].snapshot(),
            plan_number=s['plans'], planner_revision='v20_common_coverage_budget')
    ...
    def summary(self, scene_idx):
        ...
        result['v20_coverage'] = None if s is None else s['coverage_v20'].snapshot()
        result['region_continuation'] = 'not installed by common N/G capacity probe'
        return result


class FacilityRuntimeV20(FacilityRuntimeV19):
    def choose_goal(self, scene_idx, proposed_local, bounds):
        ...
        if s is not None: s['v20_last_route_update'] = s['packet'].action_id
        return result

    def next_local_action(self, scene_idx):
        s = self.states[scene_idx]
        if s is None or s['closed']: return None
        if s['pending'] is not None: raise RuntimeError('consume paid observation before V20 route update')
        ...
        if s['phase'] == 'outbound' and s['active_actions'] and s['option'] is not None and (
                p.action_id - s.get('v20_last_route_update', p.action_id) >= interval):
            ...
            if route is not None:
                mask = space.route_mask(route)
                assessment = b['coverage_v20'].assess_route(route, mask, b['ledger'].remaining_budget)
                if old.get('v20_coverage_intent'):
            ...
            s['v20_last_route_update'] = p.action_id
            backend._record(scene_idx, 'STGHP', 'refresh_observed_route_v20',
                dict(map_version=s['mapper'].frames, current_pose=[*p.position, p.heading]), event, started)
            self.audit.append(dict(event='v20_route_update', **event))
    ...
    def install_pilot_option(self, option_name):
        raise RuntimeError('V20 first validates common coverage; A/B requires a later explicit protocol')


def facility_components_v20(args, shape):
    comp = axis_components_v16(args, shape)
    comp._cpu_backend = FacilityBackendV20(args, 1, shape)
