"""V28 r1 observation-only descriptor and shortlist development prototype.
...
VERSION = 'v28-observed-debt-shortlist-preflight-r1'
CONFIDENCE_MIN = .6
...
def describe_neighborhood(state, cue):
    ...
    descriptor = dict(cue_id=cue.cue_id, action_id=state.action_id,
        patch_count=len(patches), patch_relative_directional_support=list(support),
        directional_deficit=None,
        direction_support_reference='each measured patch; not cue-centered support',
        ...
        for cue in cues:
            ...
            if count:
                info[cue.cue_id] = dict(sector=_sector(delta), visible_patches=count,
                    ...
                    novel_direction_patches=visible['novel_direction_patches'],
                    candidate_direction_deficit=visible['novel_direction_patches']/count)
...
def rank_context(context, mode='S', reserve_instances=True):
    """S=class prior; O=uniform prior at fixed detector outputs; G=geometry only.

    O conditions on the same semantic detector positions/confidence as S.
    It isolates direction-prior labels; it is not an independent object detector.
    ...
    for source in context.pool:
        ...
        for cue_id, geometry in context.cue_geometry[tuple(row['pose'])].items():
            ...
            k = geometry['sector']
            # Each patch's bit is queried using THAT patch-to-candidate vector.
            # Cue-to-candidate sector only selects the category prior.
            debt = float(weights[k]*geometry['candidate_direction_deficit'])
            ...
            if gain > 0:
                evidence.append(dict(cue_id=cue_id, class_id=cue.class_id if mode != 'O' else None,
                    ...
                    observed_direction_deficit=geometry['candidate_direction_deficit'], **{key: value for key, value
