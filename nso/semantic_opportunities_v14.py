"""Observed-map route/instance features; no fitted policy or efficacy claim.
...
def observed_descriptors(runtime, candidates):
    ...
    for pose in history:
        for ai, asset in enumerate(assets):
            prefix[ai] = np.maximum(prefix[ai], aperture_support(asset, pose, mapper.config))
    ...
    for c in candidates:
        ...
        result.append(dict(candidate_id=c['candidate_id'], remaining_budget=ledger.remaining_budget,
            ...
            return_cost=c['return_cost'], observation_horizon='outbound_only_v14',
...
def route_instance_features(mapper, candidates, observed_assets, descriptors, *, confidence_scale=1.):
    ...
    Descriptors are V14 outbound-only records, not continuation result records.
    ...
    for c, d in zip(candidates, descriptors):
        if d.get('observation_horizon') != 'outbound_only_v14':
    ...
    support = np.asarray([[float(aperture_support(a, p, config).mean()) for a in assets]
