# Versioned V16.3 copy: retain V16.2 roles; append feasible aperture viewpoints.
# Original SHA256: 7244f0044264e84c07ed0155820376e7156d1f8ef15f0c036fbfc480bee5bfb1
...
from nso.competition_candidates_v8_1 import (
    XY_DIRECTIONS, aperture_support, navigation_rear_boundary,
)
from nso.observed_asset_axes_v16 import measured_assets_v16 as measured_assets
from nso.staged_corner_views_v16 import select_corner_view
...
def generate_options(mapper, position, heading, remaining_budget, return_anchor,
    ...
    audit = {
        'schema_version': 'hierarchical_options_v16_3/1',
    ...
    # Append common geometry staging roles after every original candidate.
    # Replan after actual arrival; this is not a promised rear observation.
    for side_sign, suffix in ((-1, 'corner_minus'), (1, 'corner_plus')):
        for ai, asset in enumerate(assets):
            role = f'asset_{ai}_{suffix}'
            if len(chosen) >= limit:
                audit['omitted_by_cap'].append(role)
                audit['target_audit'].append(dict(role=role, asset_index=ai, reason='candidate_cap_reached'))
                continue
            row, record = select_corner_view(asset, side_sign, pool, used, current, mapper)
            add(row, role, ai)
            audit['target_audit'].append(dict(role=role, asset_index=ai, **record))
    # View-space proposal: actual safe camera poses, not a projection onto
    # an assumed ideal centre. This support remains a geometric hypothesis.
    for ai, asset in enumerate(assets):
        role = f'asset_{ai}_aperture_view'
        if len(chosen) >= limit:
            audit['omitted_by_cap'].append(role)
            audit['target_audit'].append(dict(role=role, asset_index=ai, reason='candidate_cap_reached'))
            continue
        options = []
        for row in pool:
            if row['state'] in used: continue
            support = float(aperture_support(asset, row['camera'], config).mean())
            if support > 0: options.append((row, support))
        selected = min(options, key=lambda pair: (-pair[1], pair[0]['outbound_cost'],
                       pair[0]['cost'], pair[0]['pose'])) if options else None
        add(None if selected is None else selected[0], role, ai)
        audit['target_audit'].append(dict(role=role, asset_index=ai,
            reason='no_unused_feasible_aperture_view' if selected is None else 'selected',
            eligible_poses=len(options), hypothetical_aperture_fraction=None if selected is None else selected[1],
            fixed_center_projection_required=False, hidden_visibility_certified=False))
    routes = []
