...
class CLIPEncoder:
    """
    轻量 CLIP 封装，缓存文本嵌入。
    当 openai-clip 未安装时自动进入离线模式（返回常数 0.5 相似度）。
    """

    def __init__(self, model_name: str = "ViT-B/32", device: str = "cpu"):
        clip = _try_import_clip()
        self.available = clip is not None
        if not self.available:
            print("[CLIPEncoder] openai-clip 未安装，使用离线模式（相似度恒为 0.5）。"
    ...
    def _get_clip(self) -> CLIPEncoder:
        if self._clip is None:
            ...
            if not self._clip.available:
                print("[OV-SDF] CLIP 不可用，语义相似度将使用 YOLOv8 置信度替代")
        return self._clip
